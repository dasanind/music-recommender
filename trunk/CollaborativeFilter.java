...
public class CollaborativeFilter {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Enter Active User Id :");
        ...
        recommendSongs(url, db, driver, user, pass, activeUser, tableName);


    }

    private static void recommendSongs(String url, String db, String driver, String user, String pass, Integer activeUser, String tableName) {
        Connection con;

        try {
             Dataset allSongs = FileHandler.loadDataset(new File("/Users/MusicDataDump/UserUnRated.txt)", ",");
            ...
            Iterator iter = allSongs.iterator();
            while (iter.hasNext()) {
                ...
                System.out.println("Rating for song " + songId + " is " + rating + " " + new Date());
                songsAndPredictedRatings.add(new Song(songId, rating));
            }

            Collections.sort(songsAndPredictedRatings, new SongComparator());

            int i = 0;
            for (Song song : songsAndPredictedRatings) {
                i = i + 1;
                System.out.println(song.getSongId() + " " + song.getMeasure());
                if (i == 30) break;
            }
    ...
    private static void storePredictedRatingsInDatabase(List<Song> songsAndPredictedRatings) {
        ...
        try {
            ...
            Statement st = con.createStatement();
            st.executeUpdate("drop table if exists CollaborativeFilterRatings");
            st.executeUpdate("create table CollaborativeFilterRatings ( songId int(15), rating double)");

            for (Song song : songsAndPredictedRatings) {
                st.executeUpdate("insert into CollaborativeFilterRatings values (" + song.getSongId() + "," + song.getMeasure() + ")");
            }
            con.close();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private static void calculate_Pearsons_Coeff(String url, String db, String driver, String user, String pass, Integer activeUser, String tableName) {
        ...
        try {
            ...
            st4.executeUpdate("DROP TABLE Pearsons_Coeff");
